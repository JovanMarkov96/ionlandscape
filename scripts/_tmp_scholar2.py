...
CACHE = 'reports/scholar_cache2.json'
...
for f, fm in targets:
    ...
    try:
        r = session.post("https://html.duckduckgo.com/html/",
                         data={"q": f'site:scholar.google.com citations "{name}"'},
    ...
    if r.status_code != 200 or 'result__a' not in r.text:
        if 'anomaly' in r.text or r.status_code in (403, 429):
            ddg_fail += 1
            print(f"  DDG throttle at {name} (fail {ddg_fail})")
            time.sleep(30)
            continue
    ...
    ddg_fail = 0
    ids = []
    for u in RE_DDG.findall(r.text):
