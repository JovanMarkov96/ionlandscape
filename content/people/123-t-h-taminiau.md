---
active: active
applications:
- computing
- networking
created_at: '2026-05-26'
current_position:
  confidence: confirmed
  institution: Delft University of Technology
  source: https://qutech.nl/person/tim-taminiau/
  title: Group Leader, QuTech
  since_year: 2015
  verified_at: '2026-05-29'
education:
- degree: PhD
  institution: ICFO – The Institute of Photonic Sciences
  year: 2011
  advisor: Niek van Hulst
  advisor_id: null
  confidence: confirmed
  source: https://www.icfo.eu/event/1695/phd-thesis-defense-tim-hugo-taminiau-optical-antennas-for-single-emitters-rsquo-/
entity_type: person
group_type: experimental
id: 123-t-h-taminiau
key_papers:
- title: Universal control and error correction in multi-qubit spin registers in diamond
  year: 2014
  doi: 10.1038/nnano.2014.2
  role: first_author
- title: 'Fault-tolerant operation of a logical qubit in a diamond quantum processor'
  year: 2022
  doi: 10.1038/s41586-022-04819-6
  role: senior_author
- title: Repeated quantum error correction on a continuously encoded qubit by real-time
    feedback
  year: 2016
  doi: 10.1038/ncomms11526
  role: corresponding
- title: A ten-qubit solid-state spin register with quantum memory up to one minute
  year: 2019
  doi: 10.1103/PhysRevX.9.031045
  role: senior_author
- title: Detection and control of individual nuclear spins using a weakly coupled electron
    spin
  year: 2012
  doi: 10.1103/PhysRevLett.109.137602
  role: first_author
keywords:
- Diamond and Carbon-based Materials Research
- Quantum Information and Cryptography
- Atomic and Subatomic Physics Research
- Plasmonic and Surface Plasmon Research
- Quantum Computing Algorithms and Architecture
- Nitrogen-vacancy centers
- Quantum error correction
- Multi-qubit spin registers
- Quantum networks
last_verified_at: '2026-05-29'
lineage_check:
  advisor_verified: false
  postdoc_verified: false
  last_checked: '2026-05-29'
links:
  group_page: https://taminiaulab.qutech.nl/
  institution_profile: https://qutech.nl/person/tim-taminiau/
  google_scholar: https://scholar.google.com/citations?user=iYyVuOoAAAAJ&hl=en
  openalex: https://openalex.org/A5014871159
  orcid: https://orcid.org/0000-0002-2355-727X
location:
  city: Delft
  country: The Netherlands
  geocode_source: openalex
  lat: 52.006672
  lon: 4.35556
  precision: city
metrics:
  citation_count: 12215
  h_index: 36
  publication_count: 127
  retrieved_at: '2026-05-26'
  source: openalex
name: Tim H. Taminiau
platforms:
- nv_center
- color_center
schema_version: 2
sort_name: Taminiau, Tim H.
updated_at: '2026-05-29'
verification_source_count: 4
---
