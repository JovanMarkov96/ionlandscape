---
active: active
affiliations:
- entity_id: null
  name: Open Quantum Design
  role: Co-founder
  type: non_profit
applications:
- computing
- simulation
created_at: '2026-01-28'
current_position:
  confidence: openalex_inferred
  institution: University of Waterloo
  since_year: null
  source: https://openalex.org/A5053336892
  title: Associate Professor
education:
- advisor: Christopher Monroe
  advisor_id: 004-christopher-monroe
  confidence: confirmed
  degree: PhD (Physics)
  institution: University of Maryland, College Park
  source: https://uwaterloo.ca/institute-for-quantum-computing/profiles/rajibul-islam
  year: 2013
entity_type: person
group_type: experimental
id: 022-rajibul-islam
ion_species:
- 138Ba+
- 171Yb+
key_papers:
- doi: 10.1038/ncomms1374
  role: co_author
  title: Onset of a quantum phase transition with a trapped ion quantum simulator
  year: 2011
- doi: 10.1038/nature15750
  role: co_author
  title: Measuring entanglement entropy in a quantum many-body system
  year: 2015
keywords:
- trapped-ion quantum simulation
- quantum many-body physics
- frustrated spin models
- quantum computation
- quantum entanglement
- Quantum Information and Cryptography
- Quantum Computing Algorithms and Architecture
- Cold Atom Physics and Bose-Einstein Condensates
- Quantum many-body systems
- Quantum optics and atomic interactions
labels:
- Experimental group
last_verified_at: '2026-05-21'
lineage_check:
  advisor_verified: true
  last_checked: '2026-05-04'
  postdoc_verified: true
links:
  google_scholar: https://scholar.google.com/citations?user=dZpP2-sAAAAJ
  group_page: https://uwaterloo.ca/ion-trap-quantum-computing/
  iqc_profile: https://uwaterloo.ca/institute-for-quantum-computing/profiles/rajibul-islam
  openalex: https://openalex.org/A5053336892
  orcid: https://orcid.org/0000-0002-6483-8932
location:
  city: Waterloo
  country: Canada
  lat: 43.4643
  lon: -80.5204
  precision: city
  region: Ontario
metrics:
  citation_count: 5363
  h_index: 21
  publication_count: 93
  retrieved_at: '2026-05-25'
  source: openalex
name: K. Rajibul Islam
platforms:
- trapped_ion
postdocs:
- advisor: Markus Greiner
  advisor_id: null
  confidence: confirmed
  institution: Harvard University (Center for Ultracold Atoms)
  note: Postdoctoral Fellow working on entanglement in ultra-cold neutral bosonic
    atoms.
  years: 2012-2015
- advisor: Vladan Vuletic
  advisor_id: 040-vladan-vuletic
  confidence: confirmed
  institution: Massachusetts Institute of Technology (Center for Ultracold Atoms)
  note: Postdoctoral Researcher working with ions in optical potentials.
  years: 2015-2016
schema_version: 2
sort_name: Islam, K. Rajibul
thesis:
  link: Unknown
  note: Received the Distinguished Dissertation Award in 2013.
  title: Quantum Simulation of Interacting Spin Models with Trapped Ions
  year: 2013
updated_at: '2026-05-25'
verification_source_count: 3
---

K. Rajibul Islam is an Associate Professor at the Institute for Quantum Computing (IQC) and the Department of Physics and Astronomy at the University of Waterloo. His research focuses on experimental quantum many-body physics, particularly the quantum simulation of frustrated spin models with trapped ions. He leads the Laboratory for Quantum Information with Trapped Ions (QITI), which is developing "QuantumIon", a multi-user, open-access trapped-ion quantum computer. He also co-founded Open Quantum Design, a non-profit foundation developing open-source, full-stack trapped-ion quantum computing blueprints.

He received his PhD from the University of Maryland (2013) under the supervision of Christopher Monroe, where his thesis on quantum simulation won the university's Distinguished Dissertation Award. He then transitioned to neutral atoms and hybrid systems as a postdoctoral fellow in Markus Greiner's group at Harvard University (2012–2015), where he helped pioneer the direct measurement of entanglement entropy in many-body systems. He followed this with a postdoctoral stint in Vladan Vuletić's group at MIT (2015–2016) before joining the faculty at Waterloo.

## Publications
See full publication list on [Google Scholar](https://scholar.google.com/citations?user=dZpP2-sAAAAJ).
