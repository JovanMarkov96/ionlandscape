---
active: active
applications:
- computing
current_position:
  confidence: reported
  institution: Griffith University
  since_year: null
  source: https://www.griffith.edu.au/
  title: Physicist (trapped-ion quantum computing)
entity_type: person
group_type: experimental
id: 261-david-kielpinski
ion_species:
- 9Be+
labels:
- Experimental group
last_verified_at: '2026-06-02'
links:
  orcid: https://orcid.org/0000-0002-2816-6892
  openalex: https://openalex.org/A5051592659
  homepage: https://www.griffith.edu.au/
location:
  city: Brisbane
  country: Australia
  lat: -27.4975
  lon: 153.0137
  precision: city
  region: Queensland
name: David Kielpinski
platforms:
- trapped_ion
postdocs:
- advisor: David J. Wineland
  confidence: reported
  institution: NIST, Boulder (Ion Storage Group)
  note: Co-developed the QCCD (quantum charge-coupled device) architecture for a scalable
    ion-trap quantum computer, using ⁹Be⁺ ions.
  source: https://tf.nist.gov/general/pdf/1566.pdf
  years: unknown
schema_version: 2
sort_name: Kielpinski, David
sources:
- note: Institutional / group page
  url: https://www.griffith.edu.au/
- note: Architecture for a large-scale ion-trap quantum computer (Kielpinski, Monroe,
    Wineland)
  url: https://tf.nist.gov/general/pdf/1566.pdf
updated_at: '2026-06-02'
verification_source_count: 1
---


David Kielpinski is a physicist known for the influential QCCD (quantum charge-coupled device) architecture for scalable trapped-ion quantum computers, developed in the NIST Ion Storage Group of David J. Wineland using ⁹Be⁺ ions (Kielpinski, Monroe & Wineland, *Nature* 2002). He later led trapped-ion and quantum-optics research at the Centre for Quantum Dynamics, Griffith University, in Brisbane, Australia.
