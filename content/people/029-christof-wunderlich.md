---
active: active
affiliations:
- name: eleQtron
  role: Co-founder
  type: company
applications:
- computing
created_at: '2026-01-30'
current_position:
  institution: University of Siegen
  since_year: null
  title: Professor
education:
- advisor: Theodor W. Hänsch
  confidence: confirmed
  degree: PhD (Physics)
  institution: Ludwig-Maximilians-University Munich
  year: 1996
entity_type: person
group_type: experimental
id: 029-christof-wunderlich
ion_species:
- 171Yb+
key_papers:
- doi: 10.1103/PhysRevLett.87.257904
  role: co_author
  title: Ion-trap quantum logic using long-wavelength radiation
  year: 2001
- doi: 10.1103/PhysRevLett.102.073004
  role: senior_author
  title: Individual Addressing of Trapped Ions and Coupling of Motional and Spin States
    Using rf Radiation
  year: 2009
- doi: 10.1038/nature10319
  role: senior_author
  title: Quantum gates and memory using microwave-dressed states
  year: 2011
- doi: 10.1126/sciadv.1600093
  role: senior_author
  title: Versatile microwave-driven trapped ion spin system for quantum information
    processing
  year: 2016
- doi: 10.1126/sciadv.1601540
  role: co_author
  title: Blueprint for a microwave trapped ion quantum computer
  year: 2017
keywords:
- trapped ions
- microwave quantum logic
- quantum computing
- magnetic gradient traps
- MAGIC
labels:
- Experimental group
last_verified_at: '2026-05-04'
lineage_check:
  advisor_verified: true
  last_checked: '2026-05-04'
  postdoc_verified: false
links:
  google_scholar: https://scholar.google.com/citations?user=g6cUqroAAAAJ
  group_page: https://www.physik.uni-siegen.de/quantenoptik/
  orcid: null
location:
  city: Siegen
  country: Germany
  lat: 50.8748
  lon: 8.0243
  precision: city
  region: North Rhine-Westphalia
name: Christof Wunderlich
platforms:
- trapped_ion
postdocs:
- advisor: null
  confidence: not_found
  institution: Unknown
  years: null
schema_version: 2
sort_name: Wunderlich, Christof
thesis:
  link: Unknown
  title: Licht-induzierte molekulare Struktur. Eine experimentelle Untersuchung.
  year: 1996
updated_at: '2026-05-04'
verification_source_count: 2
---

Christof Wunderlich leads the Quantum Optics Research Group at the University of Siegen. His group is known for pioneering work on microwave-driven quantum logic with trapped ytterbium ions and magnetic gradient traps (MAGIC) for scalable quantum information processing. He is also a co-founder of the quantum computing startup eleQtron.

He received his PhD from LMU Munich in 1996 under Theodor W. Hänsch.

## Publications
See full publication list on [Google Scholar](https://scholar.google.com/citations?user=5Wz0VTEAAAAJ).
