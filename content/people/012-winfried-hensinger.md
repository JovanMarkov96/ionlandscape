---
active: active
affiliations:
- entity_id: null
  name: Universal Quantum
  role: Co-founder, Chief Scientist and Chairman
  type: company
applications:
- computing
created_at: '2026-01-26'
current_position:
  institution: University of Sussex
  since_year: null
  title: Professor of Quantum Technologies; Director, Sussex Centre for Quantum Technologies
education:
- advisor: Halina Rubinsztein-Dunlop; Norman Heckenberg; Gerard Milburn
  confidence: confirmed
  degree: PhD (Physics)
  institution: University of Queensland
  source: https://profiles.sussex.ac.uk/p179227-winfried-hensinger
  year: 2002
entity_type: person
group_type: experimental
id: 012-winfried-hensinger
ion_species:
- 171Yb+
key_papers:
- doi: 10.1126/sciadv.1601540
  role: senior_author
  title: Blueprint for a microwave trapped ion quantum computer
  year: 2017
- doi: 10.1103/PhysRevLett.117.220501
  role: senior_author
  title: Trapped-ion quantum logic with global radiation fields
  year: 2016
keywords:
- microwave trapped-ion computing
- scalable architectures
- quantum engineering
- utility-scale quantum computing
labels:
- Experimental group
last_verified_at: '2026-05-21'
lineage_check:
  advisor_verified: true
  last_checked: '2026-05-04'
  postdoc_verified: true
links:
  google_scholar: https://scholar.google.com/citations?hl=en&user=DuOb8ioAAAAJ
  group_page: https://www.sussex.ac.uk/research/centres/ion-quantum-technology/
  nqit_profile: https://nqit.ox.ac.uk/person/prof-winfried-hensinger.html
  orcid: https://orcid.org/0000-0001-8329-438X
  sussex_profile: https://profiles.sussex.ac.uk/p179227-winfried-hensinger
location:
  city: Brighton
  country: United Kingdom
  lat: 50.8674
  lon: -0.0878
  precision: city
  region: England
metrics:
  citation_count: 2607
  h_index: 23
  publication_count: 105
  retrieved_at: '2026-05-25'
  source: openalex
name: Winfried K. Hensinger
platforms:
- trapped_ion
postdocs:
- advisor: Christopher Monroe
  advisor_id: 005-chris-monroe
  confidence: confirmed
  institution: University of Michigan
  note: FOCUS Fellow in the Monroe group.
  years: 2002-2005
schema_version: 2
sort_name: Hensinger, Winfried K.
thesis:
  link: Unknown
  title: Experimental nonlinear quantum dynamics with ultracold atoms
  year: 2002
updated_at: '2026-05-04'
verification_source_count: 3
---

Winfried K. Hensinger is a Professor of Quantum Technologies at the University of Sussex, where he leads the Sussex Ion Quantum Technology Group and directs the Sussex Centre for Quantum Technologies. He is also the Co-founder, Chief Scientist, and Chairman of Universal Quantum, a full-stack quantum computing company spun out of his lab in 2018.

His research focuses on scalable trapped-ion quantum computing, specifically pioneering the use of microwave control rather than traditional laser beams to execute quantum logic gates. His group is well known for developing practical, engineering-driven, utility-scale blueprints for modular trapped-ion quantum computers.

Prior to his role at Sussex, he completed his PhD at the University of Queensland (2002) and held a FOCUS Fellowship as a postdoctoral researcher in Christopher Monroe's group at the University of Michigan (2002–2005).

## Publications
See full publication list on [Google Scholar](https://scholar.google.com/citations?hl=en&user=DuOb8ioAAAAJ) or his [Sussex profile](https://profiles.sussex.ac.uk/p179227-winfried-hensinger).
