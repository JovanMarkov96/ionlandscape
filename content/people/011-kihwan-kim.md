---
active: active
affiliations:
- name: Tsinghua University
  type: institution
  role: Professor (2011-2025)
  entity_id: null
applications:
- computing
- simulation
- fundamental_physics
created_at: '2026-01-26'
current_position:
  institution: Institute for Basic Science (IBS), South Korea
  title: Director, Center for Trapped Ion Quantum Science
  since_year: 2025
education:
- advisor: Wonho Jhe
  degree: PhD (Physics)
  institution: Seoul National University
  year: 2004
  confidence: confirmed
  source: https://en.sedaily.com/technology/2025/12/29/quantum-scientist-kim-ki-hwan-returns-to-korea-after-decade
entity_type: person
group_type: experimental
id: 011-kihwan-kim
ion_species:
- 171Yb+
- 138Ba+
key_papers:
- title: "Tunable quantum simulation of spin models with a two-dimensional ion crystal"
  year: 2024
  doi: 10.1038/s41567-023-02378-9
  role: senior_author
- title: "Single ion qubit with estimated coherence time exceeding one hour"
  year: 2021
  doi: 10.1038/s41467-020-20330-w
  role: senior_author
- title: "Global entangling gates on arbitrary ion qubits"
  year: 2019
  doi: 10.1038/s41586-019-1428-4
  role: senior_author
- title: "Programmable quantum simulations of spin systems with trapped ions"
  year: 2021
  doi: 10.1103/RevModPhys.93.025001
  role: co_author
keywords:
- trapped-ion quantum computing
- high-fidelity gates
- quantum simulation
- scalable architectures
- 2D ion crystals
labels:
- Experimental group
last_verified_at: '2026-05-04'
lineage_check:
  advisor_verified: false
  postdoc_verified: true
  last_checked: '2026-05-04'
links:
  google_scholar: https://scholar.google.com/citations?hl=en&user=4iNn12QAAAAJ
  group_page: https://sites.google.com/view/kihwankim-lab
  note_recent_move: https://en.sedaily.com/technology/2025/12/29/quantum-scientist-kim-ki-hwan-returns-to-korea-after-decade
  orcid: null
  tsinghua_group_page: https://info.phys.tsinghua.edu.cn/kimkihwan/
location:
  city: Daejeon
  country: South Korea
  lat: 36.3741
  lon: 127.365
  region: Daejeon
name: Kihwan Kim
platforms:
- trapped_ion
postdocs:
- advisor: Rainer Blatt
  advisor_id: 017-rainer-blatt
  institution: University of Innsbruck
  years: 'unknown'
  note: Postdoctoral researcher at University of Innsbruck.
  confidence: confirmed
- advisor: Christopher Monroe
  advisor_id: 005-chris-monroe
  institution: University of Maryland (JQI)
  years: 'unknown'
  note: Postdoctoral researcher at UMD.
  confidence: confirmed
schema_version: 2
sort_name: Kim, Kihwan
thesis:
  link: null
  note: Thesis title not found in open-access sources. Research topics included quantum
    optics and parametric resonance in MOTs.
  title: null
  year: 2004
updated_at: '2026-05-04'
verification_source_count: 3
---

Kihwan Kim is a leading trapped-ion quantum computing experimentalist and Director of the Center for Trapped Ion Quantum Science at the Institute for Basic Science (IBS) in South Korea, a position he assumed in late 2025. 

Prior to returning to South Korea, he was a Professor in the Department of Physics at Tsinghua University (2011–2025), where he led a highly successful trapped-ion research group. He earned his bachelor's, master's, and doctoral degrees in physics from Seoul National University, followed by postdoctoral research at the University of Innsbruck and the University of Maryland.

His research focuses on developing trapped-ion quantum processors, including 1D linear ion chains, dual-species architectures, and 2D ion crystals. He is particularly known for high-fidelity control, multi-qubit entangling operations (such as global entangling gates on arbitrary qubits), achieving hour-long coherence times for single-ion qubits, and groundbreaking quantum simulations of spin models and bosonic dynamics.

## Publications
See full publication list on [Google Scholar](https://scholar.google.com/citations?hl=en&user=4iNn12QAAAAJ).
