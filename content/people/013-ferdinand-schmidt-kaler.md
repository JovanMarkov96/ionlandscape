---
active: active
affiliations:
- entity_id: null
  name: Helmholtz Institute Mainz
  role: Member
  type: institution
applications:
- computing
created_at: '2026-01-26'
current_position:
  institution: Johannes Gutenberg University Mainz
  since_year: 2010
  title: Full Professor (W3) of Experimental Physics
education:
- advisor: Theodor W. Hänsch
  confidence: confirmed
  degree: PhD (Physics)
  institution: Ludwig-Maximilians-University Munich
  source: https://www.uni-mainz.de/
  year: 1992
entity_type: person
group_type: experimental
id: 013-ferdinand-schmidt-kaler
ion_species:
- 40Ca+
key_papers:
- doi: 10.1038/nature01494
  role: co_author
  title: Realization of the Cirac-Zoller controlled-NOT quantum gate
  year: 2003
- doi: 10.1103/PhysRevLett.109.080501
  role: senior_author
  title: Controlling fast transport of cold trapped ions
  year: 2012
keywords:
- trapped-ion quantum computing
- segmented ion traps
- architectures
- coherence and gate performance
- fast ion transport
labels:
- Experimental group
last_verified_at: '2026-05-04'
lineage_check:
  advisor_verified: true
  last_checked: '2026-05-04'
  postdoc_verified: true
links:
  dfg_gepris_profile: https://gepris.dfg.de/gepris/person/1260598?language=en
  google_scholar: https://scholar.google.com/citations?hl=de&user=1VkTHB4AAAAJ
  group_page: https://www.quantenbit.de/
  orcid: null
location:
  city: Mainz
  country: Germany
  lat: 49.9929
  lon: 8.2473
  precision: city
  region: Rhineland-Palatinate
name: Ferdinand Schmidt-Kaler
platforms:
- trapped_ion
postdocs:
- advisor: Theodor W. Hänsch
  advisor_id: null
  confidence: confirmed
  institution: Max Planck Institute for Quantum Optics (MPQ)
  note: Postdoctoral researcher at MPQ.
  years: 1992-1993
- advisor: Serge Haroche
  advisor_id: null
  confidence: confirmed
  institution: École Normale Supérieure (LKB)
  note: Postdoctoral research assistant.
  years: 1992-1995
- advisor: Rainer Blatt
  advisor_id: 017-rainer-blatt
  confidence: confirmed
  institution: University of Innsbruck
  note: Assistant/Senior research assistant (Universitätsassistent) before professorship.
  years: 1995-2005
schema_version: 2
sort_name: Schmidt-Kaler, Ferdinand
thesis:
  link: Unknown
  note: 'German title: Hochauflösende Spektroskopie des 1S-2S Übergangs in einem kalten
    Wasserstoff- und Deuterium-Atomstrahl'
  title: High-resolution spectroscopy of the 1S-2S transition in a cold hydrogen and
    deuterium atomic beam
  year: 1992
updated_at: '2026-05-04'
verification_source_count: 3
---

Ferdinand Schmidt-Kaler is a Full Professor of Experimental Physics at Johannes Gutenberg University Mainz, where he leads the "Quantenbit AG" research group, and is a member of the Helmholtz Institute Mainz. 

His research focuses on trapped-ion quantum computing, particularly pioneering work in segmented-trap technologies, fast ion transport, scalable architectures, and improving coherence and gate performance. 

Prior to his appointment in Mainz in 2010, he was a Full Professor at the University of Ulm (2005–2010). He previously worked as a senior research assistant with Rainer Blatt at the University of Innsbruck (1995–2005), where he was a key contributor to the first realization of the Cirac-Zoller controlled-NOT quantum gate in 2003. He completed his PhD (1992) and an initial postdoc under Theodor W. Hänsch at LMU Munich/MPQ, followed by a postdoc with Serge Haroche at ENS Paris.

## Publications
See full publication list on [Google Scholar](https://scholar.google.com/citations?hl=de&user=1VkTHB4AAAAJ).
