---
abbreviations:
- UMD
aliases:
- University of Maryland
- University of Maryland, College Park
- UMD
applications_represented:
- computing
- simulation
- networking
- optical_clocks
- fundamental_physics
created_at: '2026-06-03'
entity_type: institution
focus_areas:
- Trapped-ion quantum computing and networking
- Ultracold atoms and quantum simulation
- Quantum information science and theory
- Atomic, molecular and optical (AMO) physics
group_count: null
id: i120-university-of-maryland
institution_type: university
is_dedicated_quantum_centre: false
last_verified_at: '2026-06-03'
leadership:
- name: Andrew Childs
  role: Co-Director, Joint Center for Quantum Information and Computer Science (QuICS)
  source: https://quics.umd.edu/
- name: Alexey V. Gorshkov
  person_id: 100-alexey-v-gorshkov
  role: Fellow, Joint Quantum Institute (JQI) and QuICS (NIST/UMD)
  source: https://jqi.umd.edu/
- name: Mohammad Hafezi
  role: Fellow, Joint Quantum Institute (JQI); Professor of Physics and ECE
  source: https://jqi.umd.edu/
links:
  quantum_center: https://quantum.umd.edu/
  website: https://umd.edu
  wikipedia: https://en.wikipedia.org/wiki/University_of_Maryland,_College_Park
location:
  city: College Park
  country: United States
  lat: 38.9869
  lon: -76.9426
  precision: city
  region: Maryland
media:
  logo_path: /img/institutions/i120-university-of-maryland_mark.png
mous:
- date: '2021-01-01'
  partner: IonQ
  source: https://en.wikipedia.org/wiki/IonQ
  summary: University of Maryland and IonQ launched the National Quantum Laboratory
    (Q-Lab) at College Park, providing researchers and companies hands-on access to
    IonQ's trapped-ion quantum hardware.
name: University of Maryland
national_programs:
- US National Quantum Initiative
- NSF Quantum Leap Challenge Institute for Robust Quantum Simulation (RQS)
networks:
- Joint Quantum Institute (JQI)
- Joint Center for Quantum Information and Computer Science (QuICS)
news:
- date: '2021-10-01'
  headline: University of Maryland and IonQ open the National Quantum Laboratory (Q-Lab)
    at College Park
  source: https://en.wikipedia.org/wiki/IonQ
  summary: A public–private partnership giving academic and industry users direct
    access to IonQ trapped-ion quantum computers on the UMD campus.
platforms_represented:
- neutral_atom
- nv_center
- superconducting
- trapped_ion
schema_version: 1
short_description: The University of Maryland (UMD), in College Park, Maryland, is
  a leading public research university and a major hub of U.S. quantum science. It
  hosts the Joint Quantum Institute (JQI, jointly with NIST) and the Joint Center
  for Quantum Information and Computer Science (QuICS), leads the NSF Quantum Leap
  Challenge Institute for Robust Quantum Simulation (RQS), and is the institution
  where the trapped-ion company IonQ was co-founded out of work by Christopher R.
  Monroe. With IonQ it operates the on-campus National Quantum Laboratory (Q-Lab).
sort_name: University of Maryland
updated_at: '2026-06-03'
verification_source_count: 2
---
