---
aliases:
- Oxford Ionics
- Oxford Ionics Ltd
applications:
- computing
approach:
  architecture_tags:
  - trapped_ion
  - electronic_qubit_control
  - integrated_ion_trap_chip
  - microwave_gates
  - semiconductor_fabrication
  differentiators:
  - "Electronic Qubit Control: replaces bulky laser systems with on-chip electrodes\
    \ and currents that manipulate ions via microwave fields, integrating control\
    \ onto standard semiconductor chips"
  - Ion-trap chips are fabricated in a standard commercial semiconductor foundry,
    giving a clear manufacturing-scaling path versus bespoke optics
  - Co-founders Chris Ballance and Tom Harty hold multiple world records for trapped-ion
    qubit gate and state-preparation/measurement fidelities
  - Reported the highest-performance qubits of any quantum computer on a single chip,
    achieving record two-qubit gate fidelities without individual laser beams
  elevator_pitch: Oxford Ionics builds trapped-ion quantum computers whose control
    electronics are integrated directly onto semiconductor chips ("Electronic Qubit
    Control"), replacing fragile laser setups with scalable, foundry-fabricated ion-trap
    chips.
entity_type: company
founded_year: 2019
funding:
  total_raised_usd: 83000000
  rounds:
  - amount_usd: 45000000
    date: '2022-01-01'
    notes: "~£37M raised through 2022–2023 from deep-tech and university-spinout investors."
    lead_investors:
    - Oxford Science Enterprises
    - Braavos Capital
    other_investors:
    - Lansdowne Partners
    - Prosus Ventures
    - 2xN
    round: Seed / Series A
    source: https://www.oxfordscienceenterprises.com/stories/oxford-ionics-reimagining-quantum-computing
    stage: series_a
id: c004-oxford-ionics
last_verified_at: '2026-06-02'
links:
  website: https://www.oxionics.com
  news: https://www.oxionics.com/announcements/
location:
  city: Kidlington
  country: United Kingdom
  lat: 51.826
  lon: -1.291
  precision: city
  region: Oxfordshire
media:
  logo_path: /logos/c004-oxford-ionics.png
milestones:
- claim: Oxford Ionics founded as a spinout from the University of Oxford by Dr. Chris
    Ballance and Dr. Tom Harty to commercialize chip-integrated trapped-ion quantum
    computing.
  date: '2019-01-01'
  source: https://www.balliol.ox.ac.uk/news/2025/june/alumnus-led-quantum-computing-spinout-acquired-us-firm
- claim: Raised early backing (~£37M) led by Oxford Science Enterprises and Braavos
    Capital, with Lansdowne Partners, Prosus Ventures and others, to scale its Electronic
    Qubit Control platform.
  date: '2022-01-01'
  source: https://www.oxfordscienceenterprises.com/stories/oxford-ionics-reimagining-quantum-computing
- claim: Demonstrated record qubit performance on a single integrated chip, reporting
    industry-leading two-qubit gate and state-preparation/measurement fidelities using
    on-chip electronic control rather than individual laser beams.
  date: '2024-01-01'
  source: https://postquantum.com/quantum-computing-companies/oxford-ionics/
- claim: IonQ announced an agreement to acquire Oxford Ionics for approximately $1.075
    billion (≈$1.065B in IonQ stock plus $10M cash), combining two leaders in trapped-ion
    quantum computing.
  date: '2025-06-09'
  source: https://thequantuminsider.com/2025/06/09/ionq-acquires-uk-based-oxford-ionics-for-1-075-billion/
- claim: IonQ completed the acquisition for $1.08 billion; co-founders Chris Ballance
    and Tom Harty remain to continue trapped-ion development in the UK, becoming IonQ's
    UK quantum-computing hub.
  date: '2025-09-17'
  source: https://www.oxionics.com/announcements/ionq-completes-acquisition-of-oxford-ionics-rapidly-accelerating-its-quantum-computing-roadmap/
modality: hardware
name: Oxford Ionics
offices:
- function: hq
  location:
    city: Kidlington
    country: United Kingdom
    region: Oxfordshire
  source: https://www.oxionics.com
people:
  founders:
  - name: Chris Ballance
    role: Co-founder & CEO
    source: https://www.balliol.ox.ac.uk/news/2025/june/alumnus-led-quantum-computing-spinout-acquired-us-firm
  - name: Tom Harty
    role: Co-founder & CTO
    source: https://www.balliol.ox.ac.uk/news/2025/june/alumnus-led-quantum-computing-spinout-acquired-us-firm
  spun_out_of:
  - name: University of Oxford
    spinout_year: 2019
    source: https://www.balliol.ox.ac.uk/news/2025/june/alumnus-led-quantum-computing-spinout-acquired-us-firm
platforms:
- trapped_ion
products:
- description: Trapped-ion quantum computing modules built around foundry-fabricated
    ion-trap chips with integrated Electronic Qubit Control, manipulating qubits via
    on-chip microwave fields rather than individual laser beams.
  name: Electronic Qubit Control platform
  stage: prototype
  source: https://postquantum.com/quantum-computing-companies/oxford-ionics/
short_summary: Oxford Ionics (founded 2019, Kidlington UK) is a University of Oxford
  spinout building trapped-ion quantum computers around "Electronic Qubit Control" —
  integrating qubit control onto foundry-fabricated semiconductor chips and using on-chip
  microwave fields instead of bulky laser optics. Co-founders Chris Ballance and Tom
  Harty hold multiple world records for trapped-ion gate fidelity. In September 2025
  IonQ completed its acquisition of the company for ~$1.075 billion, making Oxford
  Ionics IonQ's UK quantum-computing hub.
schema_version: 1
sort_name: Oxford Ionics
sources:
- note: Founders, Oxford spinout and acquisition (Balliol College alumni news)
  url: https://www.balliol.ox.ac.uk/news/2025/june/alumnus-led-quantum-computing-spinout-acquired-us-firm
- note: Early funding and Electronic Qubit Control approach
  url: https://www.oxfordscienceenterprises.com/stories/oxford-ionics-reimagining-quantum-computing
- note: Technology profile and record fidelities
  url: https://postquantum.com/quantum-computing-companies/oxford-ionics/
- note: IonQ $1.075B acquisition announcement, June 2025
  url: https://thequantuminsider.com/2025/06/09/ionq-acquires-uk-based-oxford-ionics-for-1-075-billion/
- note: Completion of IonQ acquisition, September 2025
  url: https://www.oxionics.com/announcements/ionq-completes-acquisition-of-oxford-ionics-rapidly-accelerating-its-quantum-computing-roadmap/
status:
  operating_status: acquired
  acquired:
    acquirer: IonQ
    acquirer_id: c002-ionq
    date: '2025-09-17'
    type: stock_and_cash
    deal_value_usd: 1075000000
    announcement_url: https://thequantuminsider.com/2025/06/09/ionq-acquires-uk-based-oxford-ionics-for-1-075-billion/
updated_at: '2026-06-02'
verification_source_count: 5
---
